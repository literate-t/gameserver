...
void VBuffer::Init()
{
	...
	// 맨 앞 4바이트는 패킷의 길이를 위한 공간
	// 가변 길이 패킷은 나중에 길이가 확정됐을 때 설정
	m_pCurMark = m_pszVBuffer + PACKET_SIZE_LENGTH;
	m_nCurBufferSize = PACKET_SIZE_LENGTH;
}

void VBuffer::GetChar(char& cCh)
{
	cCh = static_cast<unsigned char>(*m_pCurMark);
	m_pCurMark		 += 1;
	m_nCurBufferSize += 1;
}
...
{
	CopyMemory(m_pszVBuffer, (char*)& m_nCurBufferSize
		, PACKET_SIZE_LENGTH);
	CopyMemory(pDestBuffer, m_pszVBuffer, m_nCurBufferSize);
	return true;
}
